 ...
 */
export async function recordLead(email, { name, city, zip, country, source } = {}) {
  try {
    const sql = db();
    await ensureSchema();
    const e = normalizeEmail(email);
    ...
    const cut = (v, n) => (typeof v === 'string' && v.trim() ? v.trim().slice(0, n) : null);
    await sql`
      insert into leads (email, name, city, zip, country, source, first_seen, last_seen)
      values (${e}, ${cut(name, 80)}, ${cut(city, 80)}, ${cut(zip, 32)}, ${cut(country, 8)},
              ${cut(source, 60)}, now(), now())
      on conflict (email) do update set
...
export async function listLeads(limit = 1000) {
  ...
  await ensureSchema();
  return sql`
    select l.email, l.name, l.city, l.zip, l.country, l.source, l.first_seen, l.last_seen,
