...
export async function recordPageEvent({
  event, path, visitor, session, referrer, country, region, city, device
}) {
  try {
    const sql = db();
    await ensureSchema();
    ...
    await sql`
      insert into page_events (event, path, visitor, session, referrer, country, region, city, device)
...
export async function trafficStats(days = 30) {
  ...
  const [daily, totals, paths, referrers, countries, devices, cities, purchases] = await Promise.all([
    ...
    sql`select to_char(date_trunc('day', created_at at time zone 'UTC'), 'YYYY-MM-DD') as day,
               count(*) as purchases
        from customers where created_at >= ${startDay}
        group by 1 order by 1`
  ]);
