...
export async function upsertLibraryItem({ id, kind, course, category, title, description, bodyHtml, thumbKey, sort, tags, meta }) {
  const sql = db();
  await ensureSchema();
  await sql`
    insert into library (id, kind, course, category, title, description, body_html, thumb_key, sort, tags, meta)
