...
export async function getCustomer(email) {
  const sql = db();
  await ensureSchema();
  const rows = await sql`select email, status, name from customers where email = ${normalizeEmail(email)} limit 1`;
  return rows[0] || null;
}
...
export async function upsertLibraryItem({ id, kind, course, category, title, description, bodyHtml, thumbKey, sort }) {
  const sql = db();
  await ensureSchema();
  await sql`
    insert into library (id, kind, course, category, title, description, body_html, thumb_key, sort)
...
export async function listLibrary(kind) {
  ...
  await ensureSchema();
  if (kind) {
    return sql`select id, kind, course, category, title, description, thumb_key
...
export async function getLibraryItem(id) {
  const sql = db();
  await ensureSchema();
  const rows = await sql`select id, kind, course, category, title, description, body_html, thumb_key
...
export async function listCourses() {
  const sql = db();
  await ensureSchema();
  return sql`select slug, title, lesson_count,
...
export async function getCourse(slug) {
  const sql = db();
  await ensureSchema();
  const rows = await sql`select slug, title, lesson_count, data from courses where slug = ${slug} limit 1`;
  return rows[0] || null;
}
