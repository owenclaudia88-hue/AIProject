...
/**
 * Somebody clicked through to the checkout.
 *
 * Intent, not contact details — they have arrived, they have typed nothing.
 * Deliberately the shallower of the two events: there are several times more
 * of these than there are people who go on to give a name, and an ad campaign
 * cannot optimise on a handful of conversions a week.
 *
 * So "Lead" here means something looser than it does in our own records,
 * where nobody counts until they hand over an address we could actually write
 * to. Events Manager and the leads table will disagree, by design.
 *
 * No value: a lead has not paid anything, and inventing a number here would
 * put fictional revenue in the ad reports.
 */
export const sendLead = (p) => send('Lead', p, { ...PRODUCT });
...
export const CLIENT_EVENTS = {
  ...
  InitiateCheckout: sendInitiateCheckout,
  Lead: sendLead
};
